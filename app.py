    ...
    try:
        ...
        if instant_match:
            ...
            try:
                ...
                google_link_match = re.search(r'https://[^\s"\'<>]*googleusercontent\.com[^\s"\'<>]*', resp.text)
                
                if google_link_match:
                    final_url = google_link_match.group(0)
                    links.append({"name": "Instant DL (10Gbps)", "url": final_url})
                else:
                    # If no googleusercontent link found, the link might be generated by JavaScript
                    # Try requesting the /upload endpoint directly (some CDN pages use this)
                    found_link = False
                    if '/?url=' in instant_url:
                        upload_url = instant_url.replace('/?url=', '/upload?url=')
                        try:
                            resp_upload = session.get(upload_url, headers=get_headers, timeout=15)
                            google_link_match2 = re.search(r'https://[^\s"\'<>]*googleusercontent\.com[^\s"\'<>]*', resp_upload.text)
                            if google_link_match2:
                                final_url = google_link_match2.group(0)
                                links.append({"name": "Instant DL (10Gbps)", "url": final_url})
                                found_link = True
                        except Exception as e2:
                            print(f"Error trying /upload endpoint: {e2}")
                    
                    # Fallback: try other patterns if link not found yet
                    if not found_link:
                        final_url = resp.url
